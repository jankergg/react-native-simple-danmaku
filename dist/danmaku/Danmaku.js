...
const Danmaku = forwardRef((props, ref) => {
    ...
    const send = (data, time) => {
        setInitData(initData => {
            if (typeof time == 'undefined')
                time = playTime.current;
            let t = JSON.parse(JSON.stringify(initData));
            let indexTemp = 0;
            while (initData[indexTemp] && initData[indexTemp].playTime <= time) {
                indexTemp++;
            }
    ...
    //存储弹幕所在轨道
    const [tunnel, setTunnel] = useState(Array(props.data.length).fill(''));
    //弹幕高度
    ...
    useEffect(() => {
        if (!containerLayout.height)
            return;
        const defaultMaxLine = Math.floor(containerLayout.height / itemHeight);
        if (props.maxLine && props.maxLine >= 1 && props.maxLine <= defaultMaxLine)
            maxLine.current = props.maxLine;
        //0-1之间将作为百分比
        else if (props.maxLine && props.maxLine < 1 && props.maxLine > 0)
            maxLine.current = Math.ceil(props.maxLine * defaultMaxLine);
        else
            maxLine.current = defaultMaxLine;
    ...
    //存储某弹幕是否完全出现在屏幕内
    const itemInState = useRef(Array(props.data.length).fill(false));
    const setRight = (index, value) => {
    ...
    return (<View {...props}>
        ...
        let opacity = 0;
        if (props.visible)
            opacity = typeof props.itemOpacity != 'undefined' ? props.itemOpacity : defaultProps.itemOpacity;
        const commonProps = {
        ...
        if (mode == 0) {
            return <DanmakuRollItem {...commonProps} setRight={value => setRight(v.i, value)}/>;
        }
        else if (mode == 1 || mode == 2) {
            return <DanmakuFixedItem {...commonProps} initLeft={containerLayout.x + containerLayout.width / 2} positon={mode == 1 ? 'top' : 'bottom'}/>;
        }
        else {
